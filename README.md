...
**Motion feel (the washout model).** Braking/acceleration feedback separates the STEADY level you're holding from the MOTION of *getting there* — a quick stab of the brake feels like an obvious kick even before the car has actually slowed much, while holding a steady deceleration settles into a calmer, sustained background hum. Three tunables control this, adapted from classical washout/motion-cueing rig algorithms (see "References and copyright"):

- **Level response time** — how quickly the sustained/background level tracks the actual g. Higher = slower, smoother.
- **Kick smoothing time** — how quickly the "kick" (the rate-of-change transient) itself settles. Higher = a longer, softer kick.
- **Kick strength (gain)** — how hard a sudden change hits. Higher = more dramatic kicks on sudden inputs.

**Impact:** raise Kick strength if you want sharp trail-braking stabs and throttle blips to feel more dramatic; raise the two time constants if the pads feel twitchy or "buzzy" on ordinary driving and you'd rather they feel calmer and more averaged.
...
**396+ tests, 0 build warnings, single-DLL distribution** (`bin\Release\net48\` contains only `QAdvanceFeedback.dll` and its `.pdb` — every SimHub/GameReaderCommon/MahApps/Newtonsoft reference is `<Private>false</Private>`).
...
- **Nothing here has been run inside a live SimHub session, against a real game, or on real rumble hardware.** Every one of the 300+ tests is a pure, offline, hand-computed-expectation unit test (or a real-temp-file test for the persistence layer) — the actual runtime behaviour once installed in SimHub, reading a real game, and driving real shakers is unverified and cannot be verified in this development environment.
