...
namespace QAdvanceFeedback.Tests
{
    ...
    public class DispatchBranchFormulasTests
    {
        ...
        {
            var learner = new StreamingPercentileLearner(); // no observations yet -> Average() is null
        ...
        {
            var learner = new StreamingPercentileLearner();
            for (int i = 0; i < 600; i++) learner.Observe(1.0); // learned average ratio ~1.0
        ...
        [Fact]
        public void SlipFromLearnedDistribution_returns_zero_before_the_distribution_is_ready()
        {
            var learner = new StreamingPercentileLearner();
            double result = DispatchBranchFormulas.SlipFromLearnedDistribution(5.0, learner, brakePercent: 50.0, wheelRotationCapability: true, 10.0, 10.0, 10.0, 10.0);
            Assert.Equal(0.0, result);
        }
        ...
        {
            var learner = new StreamingPercentileLearner();
            for (int i = 0; i < 600; i++) learner.Observe(i % 10 + 1); // spread 1..10, nonzero, p15..p99 distinct
            double? p15 = learner.Percentile(15.0);
            double? p99 = learner.Percentile(99.0);
            Assert.NotNull(p15); Assert.NotNull(p99);
            Assert.NotEqual(p15.Value, p99.Value);

            double result = DispatchBranchFormulas.SlipFromLearnedDistribution(p99.Value, learner, brakePercent: 0.0, wheelRotationCapability: false, null, null, null, null);
        ...
        {
            var learner = new StreamingPercentileLearner();
            for (int i = 0; i < 600; i++) learner.Observe(i % 10 + 1);
